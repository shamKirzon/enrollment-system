<script lang="ts">
	...
	import { PaymentMethod } from '$lib/types/enrollment';
	...
	$: selectedValue = getSelectedValue(selected)?.label ?? 'Select a payment method...';
...
<Popover.Root bind:open let:ids>
	...
	<Popover.Content class="w-60 p-0">
		<Command.Root>
			<Command.Input placeholder="Search a payment method..." />
				...
				{#each enrollmentStatuses as status}
					<Command.Item
						...
						onSelect={(currentValue) => {
							...
							replaceSearchParam('payment_method', value);
						}}
					>
